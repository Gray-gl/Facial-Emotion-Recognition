...
class Fatigue_detecting(wx.Frame):
    ...
    def __init__(self, parent, title):
        ...
        # 表情检测部分
        sbSizer4 = wx.StaticBoxSizer(wx.StaticBox(sbSizer1.GetStaticBox(), wx.ID_ANY, u"表情检测检测"), wx.VERTICAL)
        bSizer8 = wx.BoxSizer(wx.HORIZONTAL)
        # 增加对应的按钮，实现表情校准
        self.standard_button4 = wx.Button(sbSizer4.GetStaticBox(), wx.ID_ANY, u"表情校准", wx.DefaultPosition, wx.Size(90, 25), 0)

        bSizer8.Add(self.standard_button4,0, wx.ALIGN_CENTRE, 5)
        # self.m_checkBox4 = wx.CheckBox(sbSizer4.GetStaticBox(), wx.ID_ANY, u"表情校准", wx.DefaultPosition, wx.Size(-1, 15),0)

        # self.m_checkBox4.SetValue(True)
        # bSizer8.Add(self.m_checkBox4, 0, wx.ALL, 5)
        self.m_staticText2 = wx.StaticText(sbSizer4.GetStaticBox(), wx.ID_ANY, u"检测间隔(秒):", wx.DefaultPosition,
                                           wx.Size(-1, 15), 0)
        ...
        # 开始检测按钮，这里是调用了多线程，使用learningface方法单独实现
        ...
        # 增加表情校准功能，主要是针对当前用户而言
        self.standard_button4.Bind(wx.EVT_BUTTON, self.standard_emotion)  # 关
        ...
        # 设置窗口标题的图标
        self.icon = wx.Icon('./image/123.ico', wx.BITMAP_TYPE_ICO)
        ...
        # 闪烁阈值（秒）
        self.AR_CONSEC_FRAMES_check = 10
    ...
    def _learning_face(self, event):
        ...
        self.predictor = dlib.shape_predictor(
            r"C:\Users\gray\Desktop\FacialEmotion\Facial-Emotion-Recognition\Tools\shape_predictor_68_face_landmarks.dat")
        ...
        while (self.cap.isOpened()):
            ...
            # 如果检测到人脸
            if (len(faces) != 0):
                # 检测出人脸之后，直接对人脸进行判定，这里仅仅只需要加载原来的包即可
                ############################################################
                # 这里只需要加载出对应模型，输入图片就可以进行表情识别
                ############################################################
                # cv2.putText(im_rd, "{}".format(self.hTOTAL), (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                #             (255, 255, 0), 2)
                ...
                for k, d in enumerate(faces):
                    ...
                    if self.yawn_checkBox1.GetValue() == True:
                        ...
                        if mar > self.MAR_THRESH:  # 张嘴阈值0.5
                            ...
                            if self.mCOUNTER >= self.MOUTH_AR_CONSEC_FRAMES:  # 阈值：3
                                self.mTOTAL += 1
                                # 显示，putText的参数说明，从左往右依次是
                                # image：图片。text：需要打印的字体。org：字体左下角的坐标。font：字体的类型
                                # fontSacle字体大小。thickness：字体的粗细
                        ...
                        #             (0, 0, 255), 2)
                        cv2.putText(im_rd, "Yawning: {}".format(self.mTOTAL), (140, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                    (255, 255, 0), 2)
                    else:
                        pass
                    """
                    ...
                    if self.blink_checkBox2.GetValue() == True:
                        ...
                        #             (0, 0, 255), 2)
                        cv2.putText(im_rd, "Blinks: {}".format(self.TOTAL), (320, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                    (255, 255, 0), 2)
                    else:
                        pass
                    """
                    ...
                    if self.nod_checkBox7.GetValue() == True:
                        ...
                        cv2.putText(im_rd, "Nod: {}".format(self.hTOTAL), (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                    (255, 255, 0), 2)
                    else:
                        pass
                ...
                # 没有检测到人脸
                self.oCOUNTER += 1
                cv2.putText(im_rd, "No Face", (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3, cv2.LINE_AA)
    ...
    def _standard_emotion(self,event):
        '''
            这部分主要是用来对用户的表情进行引导的，引导用户在规定的事件内进行特定表情的录制
            ，然后将视频进行保存，主动分析关键帧并进行标注，然后进行生成数据集进行强化训练，
            训练完毕之后会进行重新加载，对人的表情进行强化训练，提高识别率
        '''
        """dlib的初始化调用"""
        ...
        if self.cap.isOpened() == True:  # 返回true/false 检查初始化是否成功
            self.CAMERA_STYLE = True
            self.m_textCtrl3.AppendText(u"打开摄像头成功!!!\n")
            self.m_textCtrl3.AppendText(u"请依照屏幕右上角的提示信息做出相关表情！！\n")
        else:
            self.m_textCtrl3.AppendText(u"摄像头打开失败!!!\n")
            self.m_textCtrl3.AppendText(u"请重新调整摄像头!!!\n")
            # 显示封面图
            self.bmp.SetBitmap(wx.Bitmap(self.image_cover))
        # 成功打开视频，循环读取视频流
        #
        while (self.cap.isOpened()):
            ...
            #    图像对象，图像的三维矩阵
            flag, im_rd = self.cap.read()
            # 这里需要将顺次将图片按照提示信息进行保存
            # 取灰度
            img_gray = cv2.cvtColor(im_rd, cv2.COLOR_RGB2GRAY)

            # 这里调用的是图片的处理功能，这里就不能在生成一个新的训练程序了，这可能比较费劲

    def standard_emotion(self,event):
        """使用多线程，子线程运行后台的程序，主线程更新前台的UI，这样不会互相影响"""
        import _thread
        # 创建子线程，按钮调用这个方法，
        _thread.start_new_thread(self._standard_emotion, (event,))
...
class main_app(wx.App):
    ...
    def OnInit(self):
        # 创建wx.Frame对象
        self.frame = Fatigue_detecting(parent=None, title="Learning Emotion System")
